...
- allow the chaining of operations like [[*pass, map][map]] and [[*filter, filter-map, unique, dedup][filter]] without allocating memory between each step.
...
transducers, like [[*drop, drop-while, take, take-while][take]], can short-circuit.

Multiple transducer functions can be chained together with [[*comp, const][comp]].
...
Yield N-length windows of overlapping values. This is different from [[*segment, window, group-by][segment]]
...
in this case [[*cons, snoc, vector, string, hash-table][cons]], thus a list.
...
Some reducers, like [[*first, last, find][first]], can also force the entire transduction to
short-circuit.
...
more performant than [[*cons, snoc, vector, string, hash-table][cons]] since it performs no final reversal.
...
transduction, then use [[*drop, drop-while, take, take-while][take-while]] within your transducer chain.
...
See also the [[*uncons, concatenate, flatten][uncons]] transducer for another way to handle incoming cons cells.
...
Notice here how [[*drop, drop-while, take, take-while][drop]] is clearly applied first:
...
Here is a simplified definition of [[*first, last, find][first]]:
...
have a stream of cons cells, you can break it up with [[*uncons, concatenate, flatten][uncons]] and then collect
with [[*cons, snoc, vector, string, hash-table][cons]] as usual:
